...
class IntroductionModify extends Component { 
    constructor(props) { 
        super(props) 
        this.state = { 
            info: [],
    ...
    componentDidMount() {
        ...
        var contentData = firebase.database().ref('introductioncontent');
    ...
    handleLabelName = label => { 
        label === "introduction_subtitle_1" ? label = 'Tựa đề phụ':
        label === "introduction_title_2" ? label = 'Tựa đề 2':
        label === "introduction_title_1" ? label = 'Tựa đề 1':
        label = null
        return label
    }
    ...
    updateUser = () => { 
        ...
        .update({
            introductioncontent: this.state.info
        })
        .then(() => {
            this.saveSuccess();
            this.hideConfirm();
...
export default IntroductionModify